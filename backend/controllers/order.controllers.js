...
export const sendDeliveryOtp = async (req, res) => {
  try {
    const { orderId, shopOrderId } = req.body;
    console.log("📦 sendDeliveryOtp body:", req.body);
    ...
    const shopOrder = order.shopOrders?.id(shopOrderId);
    if (!shopOrder) {
      console.log("❌ Shop order not found for ID:", shopOrderId);
    ...
    console.log(`✅ Generated OTP for ${order.user?.email || "unknown user"}:`, otp);
    ...
    // ✅ Wrap mail send in try-catch (won’t crash even if mail fails)
    try {
      await sendDeliveryOtpMail(order.user, otp);
      console.log("📧 OTP mail sent successfully!");
    } catch (mailErr) {
      console.log("⚠️ Mail sending failed:", mailErr.message);
    }
